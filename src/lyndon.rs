...
impl LyndonWord {
    ...
    pub fn new_smallest(len: usize, max_letter: Letter) -> Self {
        ...
        vec[len-1] = 1;
        LyndonWord {vec, len, max_letter}
    }

    fn iter(&mut self, fixed_length: bool) -> LyndonWordIter<'_> {
        if !fixed_length {
            self.len = 1; // if fixed length, the smallest word is aaaaab, otherwise it's a
        }
        LyndonWordIter { lw: self, fixed_length, stop_on_next_call: false }
    }
}

struct LyndonWordIter<'a> {
    lw: &'a mut LyndonWord,
    fixed_length: bool,
...
fn test_bounded_length_lw_iterator() {
    let bllw42: Vec<Vec<Letter>> = vec![
        vec![0], vec![0, 0, 0, 1], vec![0, 0, 0, 2], vec![0, 0, 1],
        vec![0, 0, 1, 1], vec![0, 0, 1, 2], vec![0, 0, 2], vec![0, 0, 2, 1],
        vec![0, 0, 2, 2], vec![0, 1], vec![0, 1, 0, 2], vec![0, 1, 1],
        vec![0, 1, 1, 1], vec![0, 1, 1, 2], vec![0, 1, 2], vec![0, 1, 2, 1],
        vec![0, 1, 2, 2], vec![0, 2], vec![0, 2, 1], vec![0, 2, 1, 1],
        vec![0, 2, 1, 2], vec![0, 2, 2], vec![0, 2, 2, 1], vec![0, 2, 2, 2],
        vec![1], vec![1, 1, 1, 2], vec![1, 1, 2], vec![1, 1, 2, 2],
        vec![1, 2], vec![1, 2, 2], vec![1, 2, 2, 2], vec![2],
    ];
